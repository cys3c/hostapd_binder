...
struct hostapd_karma_data * get_assoc_karma_data(struct hostapd_data *hapd)
{
	...
	s = hapd->karma_data_list;
    // set the assoc bit
    while (s != NULL) {
        sta = ap_get_sta(hapd, s->mac_addr);
